...
export async function getMarketSnapshot(symbol:string,forceDemo=false):Promise<MarketSnapshot>{
  ...
  catch(error){const detail=error instanceof Error?error.message:"Unknown provider error.";return buildSnapshot(symbol,createDemoCandles(symbol),"demo",`Live data was unavailable, so demo data is shown. ${detail}`);}
}
